 ...
 * @see https://stackoverflow.com/a/30646964/19515980
*/
function rotateCounter (xs, count) {
  var n = count % xs.length;
  return xs.slice(n).concat(xs.slice(0, n));
}
...
*/
function stutterN (inputArray, stutterCounts) {
    let outputArray = [];
...
*/
function stutter (inputArray, stutterCount) {
...
*/
function replacePeriodically (inputArray, replacementItem, period) {
...
*/
function addPeriodically (inputArray, itemToAdd, period) {
    if (Number.isInteger(period) === false) {
        period = Math.floor(period)
    }
